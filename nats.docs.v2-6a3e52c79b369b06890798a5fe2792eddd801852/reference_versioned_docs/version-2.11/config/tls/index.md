...
<Reloadable state="reloadable" note="New TLS material is used by connections established after the reload; existing client connections keep the session they negotiated. `pinned_certs` is applied to existing connections immediately." />
...
| [`connection_rate_limit`](./connection_rate_limit.md) |  | `integer` | - | No\* |
