...
// transitively.
//
// ADR-001 §9's Stage 2 (triage, the template) HAS LANDED: `src/main/kotlin/adr/blocks/triage/` holds
// this block's 5 pure files. `Contract.kt` is NOT among them and never will be —
// Kotlin requires every variant of a sealed hierarchy in ONE module, so the block's
// transport lives in `:spine` under `adr/blocks/triage/` and normalises back under
// `blocks/triage/` for the gate (see spine/build.gradle.kts). The module was DECLARED
