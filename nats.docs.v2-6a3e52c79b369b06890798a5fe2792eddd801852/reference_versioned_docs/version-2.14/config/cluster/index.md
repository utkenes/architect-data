...
| [`connect_backoff`](./connect_backoff.md) | Back off between route reconnect attempts instead of retrying at a fixed interval. | `boolean` | `false` | Yes |
| [`write_deadline`](./write_deadline.md) | How long a route write may block before the connection is treated as stalled. | `duration` | `10s` | Yes |
| [`write_timeout`](./write_timeout.md) | What to do when a route misses its `write_deadline`. | `string` | `default` | Yes |
