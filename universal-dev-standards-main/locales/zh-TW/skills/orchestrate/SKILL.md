...
scope: universal
description: |
  以 Claude 原生 Agent tool 編排多任務執行計畫（以 DAG 為基礎，不需外部引擎）。
  Use when: 執行帶有平行／循序任務相依關係的 plan.json 檔案。
  Not for: 一開始產生計畫——請用 /plan；彼此之間沒有相依關係的單一任務。
  Keywords: orchestrate, plan, execute, DAG, task plan, 編排, 執行計畫, 並行, 任務相依.
