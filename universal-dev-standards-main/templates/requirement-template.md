...
### 2.4 流程規格（Flow Specification）

<!-- 🔴 含 3 個以上步驟的流程必填；純 CRUD / 單步驟操作可省略 -->
<!-- 目的：確保 PRD 在發行前就具備「測試可行性三要素」，消除 happy-path-only 的測試盲點 -->
...
```
Step 1: [動作] → Step 2: [動作] → Step 3: [動作] → ... → Step N: [動作]
```

#### 決策點清單 (Decision Points)
...
#### 終止狀態清單 (Terminal States)

| 狀態代碼 | 說明 | 對應 HTTP / 系統碼 |
|---------|------|-----------------|
| `SUCCESS` | [成功路徑說明] | 200 |
| `FAIL_[CODE_1]` | [失敗原因說明] | 4xx |
| `FAIL_[CODE_2]` | [失敗原因說明] | 4xx / 5xx |

> **驗收門檻**：每個終止狀態必須在 9.4 流程測試矩陣中至少對應一個測試情境。
...
### 9.4 流程測試矩陣 (Flow Test Matrix)

<!-- 🔴 對應 2.4 流程規格，含 3 個以上步驟的流程必填 -->
<!-- 覆蓋策略：預設 Each-Choice；認證/安全/金流 → 升級為 All-Combinations -->

#### Decision Table（Each-Choice 最小覆蓋）

| 情境 | [決策點 1] | [決策點 2] | 預期終止狀態 |
|-----|-----------|-----------|------------|
| S1  | [值]      | [值]      | `SUCCESS` |
| S2  | [值]      | [值]      | `FAIL_[CODE_1]` |
| S3  | [值]      | [值]      | `FAIL_[CODE_2]` |
...
- 決策點唯一值總數（Each-Choice 公式）：[數字]
- 目前場景數：[數字]（≥ 上方數字 ✓）
- 認證 / 安全關鍵流程 → 升級 All-Combinations：✓ / ✗

#### UAT 驗收腳本（從 Decision Table 直接衍生）

<!-- 每個 Decision Table 情境對應一列，不另起爐灶；UAT 驗證「定義正確性」，CI 驗證「覆蓋完整性」 -->

| UAT 情境 | 操作步驟摘要 | 預期終止狀態 | 測試人員 | 業務確認人 | 簽核 |
|---------|------------|------------|---------|----------|------|
| S1 — [情境名稱] | [操作摘要] | `SUCCESS` | [QA] | [業務主管] | □ |
| S2 — [情境名稱] | [操作摘要] | `FAIL_[CODE_1]` | [QA] | [業務主管] | □ |
| S3 — [情境名稱] | [操作摘要] | `FAIL_[CODE_2]` | [QA] | [業務主管] | □ |

> 若 UAT 過程中發現**未列入**的新終止狀態，須補入 §2.4 + Decision Table + 自動化測試，重跑 Gate 3 後才能繼續 UAT。

#### Flow Gate 三階段檢查清單

**Gate 0 — PRD 發行前（開始實作前，由 PM / Tech Lead 簽核）**

- [ ] §2.4 前置條件已文件化
- [ ] §2.4 步驟順序已列出
- [ ] §2.4 所有決策點已萃取
- [ ] §2.4 所有終止狀態已定義
- [ ] Decision Table 完整（每個終止狀態 ≥ 1 個情境）
- [ ] 場景數符合 Each-Choice 最小公式
- [ ] 關鍵流程（認證/安全/金流）已標注 All-Combinations
- [ ] UAT 驗收腳本欄位已預填（測試人員 + 業務確認人）

**Gate 3 — Pre-UAT 部署前（由 CI 自動驗證 + QA Lead 簽核）**

- [ ] CI：Decision Table 所有情境均有對應自動化測試且全部 green
- [ ] CI：每個終止狀態至少一個測試覆蓋（零遺漏）
- [ ] CI：branch coverage ≥ 90%（或專案設定閾值）
- [ ] 關鍵流程（認證/安全）：All-Combinations 全場景通過
- [ ] UAT 驗收腳本的「操作步驟摘要」已由 QA 補齊

**Gate 4 — UAT Sign-off（由業務確認人簽核，進入 Pre-Production）**

- [ ] 業務確認每個終止狀態的**定義正確**（與實際業務規則相符）
- [ ] UAT 驗收腳本簽核欄全部勾選
- [ ] 若 UAT 期間補入新終止狀態 → Gate 3 已重跑且通過
- [ ] 業務確認人簽核（見十四章）
